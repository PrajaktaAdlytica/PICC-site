...
return <section className={`internal-hero ${theme}`}><div className="internal-copy" data-reveal="left"><Label>{label}</Label><h1>{title}</h1><p>{text}</p></div>{photo?<Photo priority className="internal-photo" motion="scale" asset={photoAsset} alt={photoAlt} parallax={false}/>:<div className="connection-art" data-reveal="right" aria-hidden="true"><span className="art-word">PL</span><div className="art-bridge"><i/><b/><i/></div><span className="art-word">IL</span><small>PEOPLE. BUSINESS. OPPORTUNITIES.</small></div>}</section>;
}
export function Closing({title="Let’s talk business.",text='Tell us what you are looking to achieve in Poland, Israel or between the two markets.',button='Start a conversation',label='Your next connection',href='/contact'}:{title?:string;text?:string;button?:string;label?:string;href?:string}) {return <section className="closing"><div className="container closing-inner" data-reveal><div><Label>{label}</Label><h2>{title}</h2><p>{text}</p></div><Action href={href} light>{button}</Action></div><div className="closing-plane" aria-hidden="true"/></section>;}
