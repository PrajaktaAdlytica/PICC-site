...
export function Photo({className='',priority=false,caption,original=false}:{className?:string;priority?:boolean;caption?:string;original?:boolean}) {
 return <figure className={`photo ${className}`}><Image src={original?'/assets/picc-original.jpg':'/assets/architecture.png'} alt={original?'Architectural artwork from the supplied PICC visual collection':'Monochrome architectural illustration of a modern business district'} fill sizes="(max-width: 760px) 100vw, 50vw" preload={priority}/><div className="photo-blue" aria-hidden="true"/><div className="photo-red" aria-hidden="true"/>{caption&&<figcaption>{caption}</figcaption>}</figure>;
}
...
return <section className={`internal-hero ${theme}`}><div className="internal-copy"><Label>{label}</Label><h1>{title}</h1><p>{text}</p></div>{photo?<Photo priority className="internal-photo"/>:<div className="connection-art" aria-hidden="true"><span className="art-word">PL</span><div className="art-bridge"><i/><b/><i/></div><span className="art-word">IL</span><small>PEOPLE. BUSINESS. OPPORTUNITIES.</small></div>}</section>;
}
